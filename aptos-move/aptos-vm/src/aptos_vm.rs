...
use aptos_types::{
    ...
    on_chain_config::{new_epoch_event_key, CurrentTimeMicroseconds, FeatureFlag, OnChainConfig},
    transaction::{
...
impl AptosVM {
    ...
    fn success_transaction_cleanup<S: MoveResolverExt, SS: MoveResolverExt>(
        ...
        let mut user_txn_change_set_ext = user_txn_session
            .finish(&mut (), gas_meter.change_set_configs())
            .map_err(|e| e.into_vm_status())?;

        // FIXME(aldenhu)
        // n.b. assuming timestamp doesn't change in `user_txn_session`
        let timestamp_us = CurrentTimeMicroseconds::fetch_config(&storage.as_move_resolver())
            .ok_or_else(|| VMStatus::Error(StatusCode::STORAGE_ERROR))?
            .microseconds;
        let storage_deposit_charges = gas_meter.apply_storage_deposit_charges(
        ...
        self.0
            .run_storage_deposit_charges(&mut session, gas_meter, storage_deposit_charges)?;
