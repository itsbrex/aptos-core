...
use aptos_gas::{
    ...
    FromOnChainGasSchedule, Gas, NativeGasParameters, StorageDepositCharges, StorageGasParameters,
};
    ...
    pub(crate) fn run_storage_deposit_charges<S: MoveResolverExt>(
        ...
        session
            .execute_function_bypass_visibility(
                ...
                vec![],
                serialize_values(&charges.into_move_args()),
                gas_meter,
            )
            .map(|_return_vals| ())
