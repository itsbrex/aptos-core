...
const MAX_DEPTH_FROM_VALIDATORS: u64 = 100; // Required for nodes that aren't yet connected to the network
...
impl Handler {
    ...
    fn get_network_information(&self) -> Result<PeerMonitoringServiceResponse, Error> {
        ...
        let depth_from_validators =
            get_depth_from_validators(&self.base_config, self.peers_and_metadata.clone());

        // Create and send the response
        let network_information_response = NetworkInformationResponse {
            connected_peers_and_metadata,
            depth_from_validators,
        };
...
/// Returns the depth from the validators using the given base config
/// and the peers and metadata information.
fn get_depth_from_validators(
    base_config: &BaseConfig,
    peers_and_metadata: Arc<PeersAndMetadata>,
) -> u64 {
        ...
        RoleType::FullNode => {
            match peers_and_metadata.get_connected_peers_and_metadata() {
                Ok(peers_and_metadata) => {
                    // Go through our peers, find the min, and return a depth relative to that
                    let mut min_peer_depth_from_validator = MAX_DEPTH_FROM_VALIDATORS;
                    for peer_metadata in peers_and_metadata.values() {
                        if let Some(depth_from_validators) = peer_metadata
                            .get_peer_monitoring_metadata()
                            .depth_from_validators
                        {
                            min_peer_depth_from_validator =
                                min(min_peer_depth_from_validator, depth_from_validators);
                        }
                    }

                    // We're one hop away from the peer
                    min(MAX_DEPTH_FROM_VALIDATORS, min_peer_depth_from_validator + 1)
                },
                Err(error) => {
                    // Log the error and return the max depth
                    warn!(LogSchema::new(LogEntry::PeerMonitoringServiceError).error(&error.into()));
                    MAX_DEPTH_FROM_VALIDATORS
                },
            }
        },
    }
